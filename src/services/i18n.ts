...
export interface I18nServiceInterface {
  ...
  extendExpect<T extends Record<string, unknown> = Record<string, never>>(
    expect: Expect<T>
  ): Expect<T & I18nExpected>;
...
export class I18nService<L extends Locators> implements I18nServiceInterface {
  ...
  getLocalizedText(key: string, locale?: string, packageName?: string): string {
    ...
    if (translation === undefined) {
      ...
      console.warn(
        `Missing translation for key "${key}"${packageInfo} for locale "${targetLocale}"`
      );
      return `[MISSING: ${key}]`;
    }
  ...
  async switchLocale(locale: string): Promise<void> {
    ...
    try {
      ...
      if (await languageSelector.isVisible()) {
        ...
        const localeOption = this.page
          .locator(this.locators.footer.languageOption)
      ...
      if (error instanceof Error) {
        console.warn(
          "UI language switching failed, falling back to URL method:",
          error.message
        );
      } else {
        console.warn(
  ...
  extendExpect<T extends Record<string, any> = {}>(
    expect: Expect<T>
  ): Expect<T & I18nExpected> {
    ...
    return expect.extend({
      async toHaveI18nText(
        ...
        const expectedText = i18nService.getLocalizedText(
          translationKey,
          targetLocale,
          options.package
        );
        ...
        if (expectedText.startsWith("[MISSING:")) {
          ...
          return {
            message: () =>
              `Translation key "${translationKey}"${packageInfo} not found for locale "${targetLocale}"`,
            pass: false,
          };
        }
